...
def run_vitpose_landmark(working_dir, conda_env, cmd):
    """
...
if __name__ == "__main__":
    ...
    conda_env = "vitpose"
    landmark_results_dir = os.path.join(_C.VITPOSE_RESULTS_DIR, _C.SEQUENCE_NAME)
    ...
    for camera in tqdm(_C.CAMERA_NAMES, dynamic_ncols=True):
        ...
        working_dir = _C.VITPOSE_WORKING_DIR
        ...
        run_cmd = [
            "python", "-u",
            "run_fbcontact_fast.py", 
            "--img_fldr", image_dir, 
            "--bbox_fldr", bbox_dir,
            "--output_npy_pth", landmark_results_pth,
            "--visualize", "--fps", f"{_C.TARGET_FPS}"
        ]
        run_vitpose_landmark(working_dir, conda_env, run_cmd)